...
class WeibocnSpider(scrapy.Spider):
    ...
    follow_url = 'https://m.weibo.cn/api/container/getIndex?containerid=231051_-_follwers_-_{uid}&page={page}'
    fan_url = 'https://m.weibo.cn/api/container/getIndex?containerid=231051_-_fans_-_{uid}&page={page}'
    weibo_url = 'https://m.weibo.cn/api/container/getIndex?uid={uid}&type=uid&page={page}&containerid=107603{uid}'
    start_users = ['3217179555', '1742566624', '2282991915', '128739185', '3952070245', '5878659096']
    ...
    def parse_follows(self, response):
        ...
        if result.get('ok') and result.get('data').get('cards') and len(result.get('data').get('cards')) and result.get('data').get('cards')[-1].get('card_group'):
            ...
            for follow in follows:
                ...
                yield scrapy.Request(self.follow_url.format(uid=uid, page=page), callback=self.parse_follows, meta={'page': 1, 'uid': uid})
    ...
    def parse_weibos(self, response):
        ...
        if result.get('ok') and result.get('data').get('cards'):
            ...
            for weibo in weibos:
                ...
                uid = response.meta.get('uid')
                page = response.meta.get('page') + 1
                yield scrapy.Request(self.weibo_url.format(uid=uid, page=page), callback=self.parse_weibos, meta={'page': 1, 'uid': uid})
